...
async function init(data, phonemizeOnly = false) {
  ...
  // Configure ONNX Runtime - WASM only (WebGL/WebGPU don't support int64)
  ort.env.wasm.numThreads = navigator.hardwareConcurrency;
  ort.env.wasm.wasmPaths = onnxruntimeBase;
  ort.env.wasm.simd = true;

  console.log('ONNX Runtime using WASM with', navigator.hardwareConcurrency, 'threads');
  ...
  const session = cachedSession[modelUrl] ?? await ort.InferenceSession.create(
  ...
  const feeds = {
    ...
    input_lengths: new ort.Tensor("int64", new BigInt64Array([BigInt(phonemeIds.length)])),
    scales: new ort.Tensor("float32", [noiseScale, lengthScale, noiseW])
  };
  if (Object.keys(speakerIdMap).length)
    feeds.sid = new ort.Tensor("int64", new BigInt64Array([BigInt(speakerId ?? 0)]));
  const {
    output: { data: pcm }
